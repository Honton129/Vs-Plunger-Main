...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ESCAPE && !inMain)
			...
			if (passwordText.text == 'swag cool password' && FlxG.keys.justPressed.ENTER)
			{
				FlxG.sound.music.stop();
				FlxG.switchState(new SecretSongsState());
				FlxG.sound.play(Paths.sound('shock'));
			} 	
			else if (passwordText.text != 'swag cool password' && FlxG.keys.justPressed.ENTER && !inMain)
			{
		...
		if (controls.UI_LEFT_P && inMain && canMove)
			changeDiff(-1);
		if (controls.UI_RIGHT_P && inMain && canMove)
			changeDiff(1);
		...
		#if PRELOAD_ALL
		if(space && instPlaying != curSelected && inMain && canMove)
		{
		...
		else #end if (accepted && inMain && canMove)
		{
		...
		else if(controls.RESET && inMain && canMove)
		{
