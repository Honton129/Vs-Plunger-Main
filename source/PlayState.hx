...
class PlayState extends MusicBeatState
{
	...
	public static var isPlungerStage:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				case 'white': //Plunger Domains
				var bg:BGSprite = new BGSprite('whiteback', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('whitefront', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
			
				case 'hell': //Plunger Hell
				var bg:BGSprite = new BGSprite('hellback', -600, -200, 0.9, 0.9);
				add(bg);
			...
			case 'spooky': //Week 2
				...
				case 'spookyPlunger': //Spookeez Secret
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloween_bg', -200, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('halloween_bg_low', -200, -100);
			...
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					case 1:
						...
						storyPlaylist.length <= 1 && WeekData.getWeekFileName() == ('plunger') && !changedDifficulty && !usedPractice) {
					...
					case 15:
						if(Paths.formatToSongPath(SONG.song) == 'plung-test' && !usedPractice) {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
				}
